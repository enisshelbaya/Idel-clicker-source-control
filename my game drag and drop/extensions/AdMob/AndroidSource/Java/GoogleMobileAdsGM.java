...
public class GoogleMobileAdsGM extends RunnerSocial
{
	...
	public void AdMob_RewardedVideo_Load()
	{
		...
		RunnerActivity.ViewHandler.post(new Runnable()
		{
			public void run() 
			{
				RewardedAd.load(activity,rewardedAdID,AdMob_AdRequest(), new RewardedAdLoadCallback()
				{
					@Override
					public void onAdFailedToLoad(@NonNull LoadAdError loadAdError) 
					{
	...
	public void AdMob_RewardedInterstitial_Load()
	{
		if(rewardedInterstitialAd == null)
			RunnerActivity.ViewHandler.post(new Runnable()
			{
				public void run()
				{
					RewardedInterstitialAd.load(activity,rewardedInterstitialAdID,AdMob_AdRequest(), new RewardedInterstitialAdLoadCallback()
					{
						@Override
						public void onAdFailedToLoad(@NonNull LoadAdError loadAdError)
						{
	...
	public void AdMob_Targeting_MaxAdContentRating(double contentRating)
	{
		...
		{
			case 0: maxAdContentRating = RequestConfiguration.MAX_AD_CONTENT_RATING_G; break;
			case 1: maxAdContentRating = RequestConfiguration.MAX_AD_CONTENT_RATING_PG; break;
			case 2: maxAdContentRating = RequestConfiguration.MAX_AD_CONTENT_RATING_T; break;
			case 3: maxAdContentRating = RequestConfiguration.MAX_AD_CONTENT_RATING_MA; break;
		}
	}
	...
	private RequestConfiguration requestConfigurationBuilder()
	{
		...
		if(targetCOPPA)
			mRequestConfiguration = mRequestConfiguration.setTagForChildDirectedTreatment(RequestConfiguration.TAG_FOR_CHILD_DIRECTED_TREATMENT_TRUE);
		
		if(targetUnderAge)
			mRequestConfiguration = mRequestConfiguration.setTagForUnderAgeOfConsent(RequestConfiguration.TAG_FOR_UNDER_AGE_OF_CONSENT_TRUE);
	...
	{
		if (consentInformation == null) return 0; // AdMob_Consent_Type_UNKNOWN

		if (consentInformation.getConsentStatus() == ConsentInformation.ConsentStatus.OBTAINED) {
			
			Context context = RunnerJNILib.ms_context;
			if (!canShowAds(context)) return 3.0; // AdMob_Consent_Type_DECLINED
